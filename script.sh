...
libgdbm-dev libc6-dev libbz2-dev zlib1g-dev python3-dev python3-setuptools git > /dev/null 2>&1
...
sudo -u azureuser ./config.sh --unattended --url ${url} --auth pat --token ${pat} --pool ${pool} --agent ${agent} --acceptTeeEula --work /home/azureuser/agent/_work --runAsService
